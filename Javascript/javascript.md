...
点击的某一个元素的时候，其祖先元素的事件是遵循先捕获再冒泡，但是在本元素上的事件是按照代码顺序执行的，与冒泡和捕获无关，写在前面的先执行，所以结论是：绑定在被点击元素的事件是按照代码顺序发生，其他元素通过冒泡或者捕获“感知”的事件，按照W3C的标准，先发生捕获事件，后发生冒泡事件。
##### 所有事件的顺序是：
###### 其他元素捕获阶段事件 -> 本元素代码顺序事件 -> 其他元素冒泡阶段事件 。<br/>
